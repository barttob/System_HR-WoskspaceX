...
const Opiekunowie = () => {
    ...
    const SiteButtons = () => {
      ...
      /*
    const sendSupervisorData = async () => {
      try {
        const response = await Axios.post(
          "http://localhost:3001/pracownicy/supervisor",
          {
            sv_id: sv_id,
            user_id: user_id,
          }
        ...
        if (response.data.success) {
          ...
          setErrorMessage("Nie przypisano pracownika. Spróbuj ponownie.");
        }
      } catch (error) {
    ...
    */
    useEffect(() => {
      ...
      const getSv = () => {
        axios.get(`http://localhost:3001/supervisor/${svSites}`).then((response) => {
